...
#settings
MOFAobject <- MOFA2::create_mofa(list("RNA" = D_expr_MOFA, "DNAm" = D_met_MOFA, "WGS" = D_WGS_MOFA))
data_optsB <- MOFA2::get_default_data_options(MOFAobject)
model_optsB <- MOFA2::get_default_model_options(MOFAobject)
model_optsB$num_factors <- 10
train_opts <- MOFA2::get_default_training_options(MOFAobject)
train_opts$convergence_mode <- "slow"
stochastic_opts <- MOFA2::get_default_stochastic_options(MOFAobject)

# Prepare
MOFAobject <- MOFA2::prepare_mofa(object=MOFAobject, data_options=data_optsB, model_options=model_optsB, training_options=train_opts)
save(MOFAobject, file = paste0(output_directory,"MOFAobject.RData"))

#run
MOFAobject.trained <- MOFA2::run_mofa(MOFAobject, save_data = T, outfile = paste0(output_directory,"MOFAobject_trained.hdf5"),use_basilisk = T)
